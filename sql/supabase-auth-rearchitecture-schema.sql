...
create table if not exists public.audit_log (
  ...
  -- "on delete set null", not cascade like every other project_id FK in
  -- this codebase: audit_log is append-only by design (see comment above),
  -- so deleting a project must never delete its audit history along with
  -- it -- only the now-dangling project_id reference is cleared, the row
  -- itself (action/entity_type/entity_id/metadata/created_at) survives.
  -- See supabase-audit-log-project-delete-fix-setup.sql for the migration
  -- that applies this to a database where audit_log already exists.
...
declare
  v_project_id uuid;
begin
  -- p_project_id can point at a project that's already been deleted earlier
  -- in THIS SAME transaction -- e.g. a schedule_task or project_members row
  -- getting cascade-deleted as part of deleting the project it belonged to,
  -- whose own AFTER DELETE audit trigger calls this with old.project_id.
  -- audit_log.project_id is "on delete set null" (see the comment on that
  -- column) so an EXISTING row degrades gracefully when its project is
  -- deleted later -- but a BRAND NEW row being inserted here would instead
  -- get rejected outright by that same foreign key, since the project is
  -- already gone by the time this INSERT runs, blocking the whole delete.
  -- Store null instead of failing, same choice as the FK makes for a row
  -- that already exists, just applied at write time too. See
  -- supabase-audit-log-write-during-project-delete-fix-setup.sql for the
  -- migration that applies this to a database where this function already
  -- exists.
